terraform {
  ...
  # Backend (configure per environment)
  backend "s3" {
    # bucket, key, region, dynamodb_table, encrypt set via -backend-config or partial config
    # See environments/dev/terraform.tfvars for per-env values
  }
}
